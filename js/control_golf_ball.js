...
function CourseBall(fX, fY, fPlayer, fCup) {
	...
	// Unique obstacle interaction.
	this.obsTrail = 0;
	this.obsKabo = undefined;
	...
	this.Stopped = function() {
		return(this.Grounded() && this.ballSpeed == 0 && this.ballStall == false);
	}
	...
	this.Move = function() {
		...
		// Grounded.
			...
			// Cup interaction.
			if (valueBetween(this.cupObj.x - 5, this.x, this.cupObj.x + 5) && valueBetween(this.cupObj.y - 4, this.y, this.cupObj.y + 3)) {
				// Thing.
				if (!this.ballSkipped) {
		...
		// Trail.
		if (this.ballTrail) {
			if (this.obsTrail > 0) tColor = 2 + this.obsTrail;
			else tColor = 1 + ((this.shotSpin != 0) * Math.sign(this.shotSpin));
		...
		// Stopped and last check for obstacles.
		if (this.Stopped()) {
			if (!ObsCollision(this)) {
				this.ballDrop = this.ballDropDefault;
			}
